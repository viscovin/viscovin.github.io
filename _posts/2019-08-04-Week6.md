...
<mark>Software should be written to deal with every conceivable error, no matter how unlikely; sooner or later 
...
entities that will send in packets designed to have the worst possible effect.</mark>
This assumption will lead to suitable protective design, although the *most serious problems in the 
...
<mark>An undefined code might be logged (see below), but it must not cause a failure. 
